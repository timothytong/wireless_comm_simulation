function [beginning_of_data, phase_of_peak, mag_channel] = frame_sync(rx_signal, os_factor)
...
current_peak_value = 5;
samples_after_threshold = os_factor;

for detection_threshold = 15:-1:10
    for i = os_factor * frame_sync_length + 1 : length(rx_signal)
        ...
        if (T > detection_threshold || samples_after_threshold < os_factor)
            ...
            if (T > current_peak_value)
                ...
                mag_channel = abs(c) / frame_sync_length;
            end
    ...
    warning('No synchronization sequence found. Decreasing threshold to %d!', detection_threshold-1);
end
